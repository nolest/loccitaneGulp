/**
 * @author nolest 2019-1
 * nowEdit为编辑面板的唯一数据绑定对象**
 */
...
var nroster = new Vue({
    ...
    methods: {
        inputChangeShop(index) {
            let that = this;
            //console.log(12345)
            //console.log(that.editData[index])
            //console.log('选前' + JSON.stringify(that.nowEdit[index].defaultRoster))
            let list = that.editData;
            if (list.length == 0) return
            for (let i = 0; i < list[index].shopList.length; i++) {
                if (list[index].shopList[i].value == that.nowEdit[index].defaultShopId) {
                    //console.log('赋值' + JSON.stringify(list[index].shopList[i].rosterDefault))
                    that.nowEdit[index].defaultRoster = list[index].shopList[i].rosterDefault
                    //console.log('在changeYour设置了that.nowEdit['+index+'].defaultRoster为'+ JSON.stringify(list[index].shopList[i].rosterDefault) 
                }
            }
        },
        /**
        ...
        */
        fetchEditData(callback) {
            ...
            axios.post(env + '/roster/getShopAndRosterInfo', {
                ...
                .then(function (response) {
                    var list = response.data.messages.allShopList;
                    //设置shopList rosterList
                    //console.log('setEditData')
                    //console.log(JSON.stringify(list))
                    that.editData = JSON.parse(JSON.stringify(list));
                    let forms = $('.form-control-hang');
                    for (let i = 0; i < list.length; i++) {
                        for (let k = 0; k < list[i].shopList.length; k++) {
                            if (list[i].shopList[k].value == list[i].currentShopId) {
                                //标记是否有用户编辑数据
                                let setST = false,
                                    setET = false,
                                    setLST = false,
                                    setLET = false;
                                //缓存用户编辑数据
                                let rosterTemp = JSON.parse(JSON.stringify(that.nowEdit[i]));
                                that.nowEdit[i].workingStartTime ? setST = true : '';
                                that.nowEdit[i].workingEndTime ? setET = true : '';
                                that.nowEdit[i].lunchStartTime ? setLST = true : '';
                                that.nowEdit[i].lunchEndTime ? setLET = true : '';
                                //赋值模板数据，会触发@input事件
                                that.nowEdit[i].defaultRoster = list[i].shopList[k].rosterDefault;
                                //停顿js解析器，等待nowEdit赋值完成
                                //假如有数据，用户数据覆盖模板数据
                                setTimeout(function () {
                                    setST ? forms.eq(i * 4).val(rosterTemp.workingStartTime) : '';
                                    setET ? forms.eq(i * 4 + 1).val(rosterTemp.workingEndTime) : '';
                                    setLST ? forms.eq(i * 4 + 2).val(rosterTemp.lunchStartTime) : '';
                                    setLET ? forms.eq(i * 4 + 3).val(rosterTemp.lunchEndTime) : '';
                                }, 0)
                                //console.log('在fetchEditData设置了that.nowEdit['+i+'].defaultRoster为'+ JSON.stringify(list[i].shopList[k].rosterDefault))
                                //console.log('在fetchEditData设置了rosterTemp['+i+']'+ JSON.stringify(rosterTemp))
                            }
                        }
                    }
        ...
        */
        setRowTime(row) {
            let that = this;
            let forms = $('.form-control-hang');
            //console.log('come to set:' + row)
            //console.log(JSON.stringify(that.nowEdit[row]))
            let list = that.editData;
            let obj = {};
            //循环对比*当前*选中的商店defaultShopId，不是接口返回的默认商店
            for (let k = 0; k < list[row].shopList.length; k++) {
                if (list[row].shopList[k].value == that.nowEdit[row].defaultShopId) { //list[row].currentShopId
                    obj = list[row].shopList[k].rosterDefault
                }
            }
            //console.log(that.nowEdit[row].defaultRoster.workingStartTime)
            forms.eq(row * 4).val(that.nowEdit[row].defaultRoster.workingStartTime || obj.workingStartTime)
            forms.eq(row * 4 + 1).val(that.nowEdit[row].defaultRoster.workingEndTime || obj.workingEndTime)
            forms.eq(row * 4 + 2).val(that.nowEdit[row].defaultRoster.lunchStartTime || obj.lunchStartTime)
            forms.eq(row * 4 + 3).val(that.nowEdit[row].defaultRoster.lunchEndTime || obj.lunchEndTime)
        },
        /**
